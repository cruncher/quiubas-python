...
class network:
    ...
    def request(self, path, params=dict(), method='GET'):
        ...
        auth_info = auth.HTTPBasicAuth(
            auth_info['api_key'], auth_info['api_private'])
        ...
        try:
            if method == 'GET':
                ...
                request = request_method(
                    url, auth=auth_info, timeout=timeout, headers=headers, verify=False, json=params)
        except ValueError as e:
            raise error(
                'There was an error trying communicating with Quiubas Server: ' + str(e))
